...
Describe "LOS Phase 2.6 Governance Enforcement" {
    BeforeAll {
        ...
        $script:moduleRoot = Join-Path $script:repoRoot "los\modules"

        Import-Module (Join-Path $script:moduleRoot "LOS.ContractEnforcer.psm1") -Force -Global
    ...
    It "allows a valid contract" {
        ...
        [string]::IsNullOrWhiteSpace($result.ContractHash) | Should -BeFalse
    }
    ...
    It "generates canonical provenance without certification fields" {
        ...
        $provenance.PSObject.Properties.Name -contains "Certification" | Should -BeFalse
    }
    ...
    It "emits structured JSON compliance reports" {
        ...
        $report.Coverage.ContractCoverage | Should -BeTrue
        $report.Coverage.SchemaCoverage | Should -BeTrue
        $report.Coverage.AttestationCoverage | Should -BeTrue
        $report.Coverage.PolicyCoverage | Should -BeTrue
        $report.Coverage.BrokerCoverage | Should -BeTrue
    }
}
