...
Describe "LOS Phase 2.6 Governance Enforcement" {
    It "allows a valid contract" {
        $result = Test-LosContractEnforcement -ContractId "EKOS.Execute" -ContractVersion "1.0.0" -ExpectedSchemaId "execution-result" -ExpectedSchemaVersion "1.0.0" -RootPath $repoRoot
    ...
    It "denies a missing contract" {
        $result = Test-LosContractEnforcement -ContractId "EKOS.Missing" -ContractVersion "1.0.0" -RootPath $repoRoot
    ...
    It "denies broker execution when any gate fails" {
        $result = Invoke-LosExecutionBroker -ContractId "EKOS.Missing" -ContractVersion "1.0.0" -Runtime "PS7" -RootPath $repoRoot
    ...
    It "emits structured JSON compliance reports" {
        ...
        $brokerResult = Invoke-LosExecutionBroker -ContractId "EKOS.Execute" -ContractVersion "1.0.0" -Runtime "PS7" -TimestampUtc "2026-06-23T00:00:00.0000000Z" -RootPath $repoRoot
